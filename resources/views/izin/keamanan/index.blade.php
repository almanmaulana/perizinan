<x-app-layout>
    ...
    <style>
        ...
        <div class="me-5 flex justify-end pe-5">
            <a href="{{ route('izin.keamanan.trash') }}" 
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8">
        <div class="max-w-6xl mx-auto bg-gray-100 dark:bg-gray-800 shadow-xl rounded-xl p-5">
            ...
            <form method="GET" id="filterForm" class="flex flex-wrap gap-2 mb-4 items-center bg-white dark:bg-gray-700 p-4 rounded-xl shadow-inner">
                ...
                <input type="date" name="simulasi" id="simulasi" class="border rounded px-2 py-1 text-sm dark:bg-gray-600 dark:border-gray-500 dark:text-gray-100"
                       value="{{ request('simulasi')  }}">
            </form>
                    ...
                    <tbody class="divide-y">
                        @forelse($izinList as $izin)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                                ...
                                <td class="px-3 py-2 align-top">{{ $izin->status=='disetujui_keamanan' ? $izin->durasi : '-' }}</td>
                                <td class="px-3 py-2 align-top">
                                    @php
                                        $statusMap = [
                                ...
                                <td class="px-3 py-2 font-bold align-top">
                                    ...
                                    @else
                                        <span class="{{ $warna }}">{{ $denda>0 ? 'Rp '.number_format($denda,0,',','.') : '-' }}</span>
                                    @endif
                                </td>

                                {{-- Kolom Aksi dengan Soft Delete / Restore / Force Delete --}}
                                    ...
                                    @else
                                        @if($izin->status=='pending_keamanan')
                                            <div class="flex flex-col gap-2 items-center">
                                                <form action="{{ route('izin.keamanan.approve',$izin->id) }}" method="POST" class="flex gap-1 items-center">
                                                    @csrf
                                                ...
                                                <form action="{{ route('izin.keamanan.reject',$izin->id) }}" method="POST" class="flex gap-1 items-center">
                                                    @csrf
                                                    <input type="text" name="catatan" placeholder="Catatan" class="border rounded px-1 py-1 text-xs">
