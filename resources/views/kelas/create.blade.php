    ...
    <div class="max-w-lg mx-auto mt-10 bg-white dark:bg-[#2a2a2a] shadow-lg rounded-2xl p-6 transition duration-300">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">
                <i class="fa-solid fa-graduation-cap text-blue-500 mr-2"></i> Tambah Kelas
            </h1>
        </div>
        ...
        <form action="{{ route('kelas.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="nama_kelas" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <i class="fa-solid fa-chalkboard mr-1 text-blue-500"></i> Nama Kelas
                </label>
            ...
            <div>
                <label for="tingkat" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <i class="fa-solid fa-layer-group mr-1 text-blue-500"></i> Tingkat
                </label>
            ...
            <div id="jurusan-wrapper" style="display:none">
                <label for="jurusan" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <i class="fa-solid fa-briefcase mr-1 text-blue-500"></i> Jurusan
                </label>
            ...
            <div>
                <label for="wali_kelas_id" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <i class="fa-solid fa-user-tie mr-1 text-blue-500"></i> Wali Kelas
                </label>
                <select name="wali_kelas_id" id="wali_kelas_id"
                        class="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-[#3a3a3a] 
                               rounded-lg p-2 text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-400 focus:outline-none transition" required>
                    <option value="">-- Pilih Wali Kelas --</option>
                    @foreach($waliKelasList as $wali)
                        <option value="{{ $wali->id }}" {{ old('wali_kelas_id') == $wali->id ? 'selected' : '' }}>
                            {{ $wali->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            {{-- Tombol Simpan --}}
            <button type="submit"
                    class="w-full mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg 
